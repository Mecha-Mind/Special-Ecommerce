...
const makeCartKey= (id:string, colorId: string, size: string) => `${id}__${colorId}__${size}`;
...
type CartContextType = {
    ...
    // helpers for UI decisions
    makeKey: ( product: Product, opts: {colorId: string; size:string;}) => string;
    getQuantity: (cartKey: string) => number;
    canAddItem: (product: Product, opts: {colorId: string; size:string;}) => boolean;
...
export function CartProvider({children}: {children:React.ReactNode}) {
    ...
    // ------------- helpers -------------
    const makeKey = ( product: Product, opts: {colorId: string; size:string;}) => makeCartKey(product.id, opts.colorId, opts.size);
    const getQuantity = (cartKey: string) => items.find((i) => i.cartKey === cartKey)?.quantity ?? 0;
    const canAddItem = (product: Product, opts: {colorId: string; size:string;}) => product.stock > 0 && getQuantity(makeKey(product, opts)) < product.stock;

    // ------------- actions -------------
    const addItem = (product: Product, opts: {colorId: string; size: string}) => {
        setItems((prev)=>{
            ...
            return [...prev, {...product, quantity: 1, selectedColorId: opts.colorId, selectedSize: opts.size, cartKey},];
        });
    };
